...
执行 `monitorenter`
1、如果 monitor 的进入数为 0，则线程进入 monitor 并将进入数设置为 1，线程取得 monitor 的所有权。
2、如果线程已经占有 monitor，则线程重新进入，将进入数 +1
3、如果 monitor 已经被其他线程取得，则线程进入阻塞状态，直到进入数为 0，再重新尝试获取 monitor 的所有权
